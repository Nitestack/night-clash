...
const StatsTracker: NextPageWithConfiguration = () => {
    ...
    useEffect(() => {
        ...
        setDefaultIndex(
            tracker.includes(location.hash.replace(/#/g, ""))
                ? tracker.indexOf(location.hash.replace(/#/g, ""))
                : 0
        );
    }, []);
    return (
        <>
            <Tabs
                ...
                tabs={{
                    "Clash of Clans": (
                        <Tab backgroundImageName="Clash of Clans" hash="home" />
                    ),
                    "Clash Royale": <Tab backgroundImageName="Clash Royale" />,
                    "Brawl Stars": (
                        <Tab backgroundImageName="Brawl Stars" club />
                    ),
                }}
                onTabChange={(index) => {
...
const Tab: FC<{
    ...
    function redirectToTracker(element: string) {
        return () => {
            if (playerTagRef.current) {
                ...
                if (
                    tag &&
                    tag.length < Util.Constants.MAXED_TAG_LENGTH &&
                    tag.length >= Util.Constants.MIN_TAG_LENGTH
                ) {
                    window.open(
    ...
    for (const element of ["player", "clan"]) {
        tabs[
            `Find a ${element == "clan" ? (club ? "club" : element) : element}`
        ] = (
            <>
                <Center>
                    <img
                        className="w-[60vw]"
                        src={`/Images/${backgroundImageName}.png`}
                    />
                </Center>
                <Center>
                    <PlayerTagInput
